...
- 差异化角色依赖：06-10所有handoff；同层01/02；路线投稿最低证据包。
- 相关性说明：全路线最终证据Gate，只裁决ready/not_ready/stop。
...
## 4. 本文件唯一变量与禁止重开项
- 唯一变量：不运行实验、不承诺录用，只判断证据是否足以支持有限主张。
...
## 5. 核心规则卡片
- 当前结论：Gate检查真实性、盲态、lineage、三seed、双benchmark、外部身份、统计、成本、失败案例和限制；B1 warning必须保留。
...
- 核心公式或定义参考：`ready = all_required_evidence ∧ no_leakage ∧ traceable ∧ claims_bounded`; 否则not_ready或stop。
- 代码落点：`scripts/summarize_stage.py::evaluate_final_gate()` planned接口；输入stage manifests，输出gate report/handoff。
- 运行记录字段：见第7节。
- 验收方式：最低包全部可回链且无关键冲突；正向结果不是证据完整性的替代品。
...
- 接口：`scripts/summarize_stage.py::evaluate_final_gate()` planned接口；输入stage manifests，输出gate report/handoff。
...
consumer_file: 11_总验收与止损/00_阶段总协议.md
...
## 9. 验收、独立复核与handoff
- 验收：最低包全部可回链且无关键冲突；正向结果不是证据完整性的替代品。
...
## 10. 独立回退条件
缺证据回对应阶段；无正向结果按01收缩/停止，不追加第四模块。
