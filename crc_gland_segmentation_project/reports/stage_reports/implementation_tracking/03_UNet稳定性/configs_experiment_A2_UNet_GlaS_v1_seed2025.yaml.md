# configs/experiment/A2_UNet_GlaS_v1_seed2025.yaml 怎么看

> 结论先行：这个文件是 A2 稳定性阶段第三个 seed 的实验配置，和 seed3407 完全同构，唯一合法差异只有 `train_seed` 和派生的 `run_name`。
> 你可以把它理解成"三次抽样里的最后一抽"——它和另外两次一起，才凑齐 n=3 的 mean±std。

## 这个文件是干什么的

这个 YAML 是三次重复实验里的第三次运行入口。

它告诉训练入口 `scripts/train.py`：这次用 seed=2025，其余数据、模型、训练、评估四套子配置全部复用 A2 冻结口径。

用人话说，它存在的唯一理由就是"在只改 seed 的前提下再跑一次"，好让 `scripts/summarize_stage.py` 能把三次结果聚合成 mean±std。
...
seed 字段回答"这是第几次"：train_seed=2025 是本文件唯一的实验变量，result_tag 与 aggregation 标注它是三次重复实验里的第三次。

config_refs 块回答"参数在哪"：它把 data/model/train/eval 四条链解引用到与 seed3407 完全相同的子配置。下面是文件的完整骨架：

```text
run_name: A2_UNet_GlaS_seed2025
stage_code: A2
dataset_code: glas
model_name: unet
config_version: v1
...
smoke_check_run_name: A2_UNet_GlaS_seed2025_smoke
config_refs:
...
这次运行的正式产物落在 `experiments/A2_UNet_GlaS_seed2025/checkpoints/best.ckpt`；文件名中的 `v1` 仅表示配置文件版本，不是 run_name。

它在两个 hold-out split 上的真实数值是：

| split | F1 | Object Dice |
|-------|-----|-------------|
| testA | 0.4781767624489242 | 0.639703471673028 |
| testB | 0.5720981061422237 | 0.7591453514828673 |

这三行数值和 `reports/tables/unet_seed_results.csv` 里 seed=2025 的对应行完全一致。

## 这些列/字段分别是什么意思

逐字段说明如下，每个字段都能在配置里核对：

- `run_name`：决定输出目录名，本次为 A2_UNet_GlaS_seed2025；配置文件名中的 `v1` 仅表示配置版本。
- `stage_code`：阶段代号 A2，标识这是 03 稳定性阶段的正式运行
- `train_seed`：本文件唯一实质变量，值为 2025
...
| 字段 | seed3407 | 本文件 |
|------|----------|--------|
| run_name | A2_UNet_GlaS_seed3407 | A2_UNet_GlaS_seed2025 |
| train_seed | 3407 | 2025 |
| smoke_check_run_name | ..._seed3407_smoke | ..._seed2025_smoke |
...
它说明的是"第三次重复运行的身份和入口"。它的上游是 A1 冻结的四套子配置，下游是 `scripts/train.py` 和 `scripts/summarize_stage.py`。

## 这个文件没说明什么

- 不规定训练超参数（那在 `configs/train/unet_flow_v1.yaml` 里）
- 不规定评估口径（那在 `configs/eval/eval_proto_v1.yaml` 里）
- 不代表"seed=2025 是更好的配置"——当前 testA F1=0.4781767624489242，不能据此挑选 seed
...
diff <(grep -v train_seed configs/experiment/A2_UNet_GlaS_v1_seed3407.yaml) \
     <(grep -v train_seed configs/experiment/A2_UNet_GlaS_v1_seed2025.yaml)
```

### 验证步骤 2：确认 checkpoint 存在
- 操作：查看 `experiments/A2_UNet_GlaS_seed2025/checkpoints/best.ckpt` 是否存在且大小大于 0
- 期望结果：存在且大小 > 0
- 实际结果：存在，可被下游评估脚本加载

## 常见问题

### Q：seed=2025 的 testA F1 最高，是不是说明 2025 是最优 seed？

A：容易误解。A2 的目标是验证稳定性，不是挑最优 seed。当前 testA F1=0.4781767624489242，不能当成“配置更好”的证据。

### Q：三个 seed 都跑完了，为什么还要单独一份聚合脚本？

A：因为单个 config 只负责一次运行，跨 seed 的 mean±std 和协议一致性校验必须由 `scripts/summarize_stage.py` 统一完成。
