...
- 差异化角色依赖：07训练协议；同层00/02；下游10统计。
- 相关性说明：执行细则，突出尺度参数来源和GlaS参数非默认继承。
...
## 4. 本文件唯一变量与禁止重开项
- 唯一变量：允许CRAG train/val内选择尺度相关参数，禁止结构搜索。
...
## 5. 核心规则卡片
- 当前结论：有pixel spacing则物理尺度换算；无则用train153/val20预注册离散候选；contour width、min area、morph kernel分别记录来源。
...
- 核心公式或定义参考：物理换算 `p_crag=round(p_glas·spacing_glas/spacing_crag)`；无spacing时只以train/val对象尺度分位数构造有限候选。
- 代码落点：`src/data/scale_adapter.py::derive_scale_config(train_meta,val_meta)`；输入元数据/掩膜统计，不接收test40；输出scale_config digest。
- 运行记录字段：见第7节。
- 验收方式：函数签名和访问控制拒绝test split；最终scale在test40前冻结；同一配置用于所有正式seed。
...
- 接口：`src/data/scale_adapter.py::derive_scale_config(train_meta,val_meta)`；输入元数据/掩膜统计，不接收test40；输出scale_config digest。
...
consumer_file: 08_CRAG验证/01_训练评估与尺度协议.md
...
## 9. 验收、独立复核与handoff
- 验收：函数签名和访问控制拒绝test split；最终scale在test40前冻结；同一配置用于所有正式seed。
...
## 10. 独立回退条件
尺度证据不足则使用明确记录的无尺度默认并降级解释，或停止08；不得偷用test40。
