...
- 差异化角色依赖：05全部协议；同层01-03；下游07全部、10汇总、11总验收。
- 相关性说明：阶段总览，广泛约束候选注册、预算、val17唯一选择和06→07交接。
...
## 5. 核心规则卡片
- 当前结论：候选全集必须在首个 run 前注册；总预算按 N1/N2/N3 分桶，N3最多12组；只用 train68/val17、seed3407。
...
- 核心公式或定义参考：`eligible = registered ∧ within_budget ∧ val17_only ∧ no_test_access`; decision matrix按合法性、对象三主指标、四补充指标、机制证据、成本和失败案例裁决。
- 代码落点：`scripts/screen_candidates.py::screen_registered_candidates(registry, budget, val_loader)`；输入候选配置与 `B×3×512×512`，输出 planned manifest/decision schema；概率 float32、mask uint8。
- 运行记录字段：见第7节。
- 验收方式：候选注册完整、预算未超、唯一 winner/收缩项可解释、Test blind工程阻断可审计后才可计划 handoff。
...
- 接口：`scripts/screen_candidates.py::screen_registered_candidates(registry, budget, val_loader)`；输入候选配置与 `B×3×512×512`，输出 planned manifest/decision schema；概率 float32、mask uint8。
...
consumer_file: 06_val筛查/00_阶段总协议.md
...
## 9. 验收、独立复核与handoff
- 验收：候选注册完整、预算未超、唯一 winner/收缩项可解释、Test blind工程阻断可审计后才可计划 handoff。
...
## 10. 独立回退条件
任何未注册候选、预算超限、val17以外反馈或多 winner，回到06阶段锁定；不进入07。
