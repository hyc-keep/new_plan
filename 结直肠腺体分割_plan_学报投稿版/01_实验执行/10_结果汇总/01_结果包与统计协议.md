# 10 raw/aggregate分层、表图schema与缺失值协议
...
- 差异化角色依赖：07统计schema、08/09 manifests；同层00/02；下游11最低包。
- 相关性说明：结果包细则，定义机器表、论文表和图的消费关系。
...
## 5. 核心规则卡片
- 当前结论：raw层逐样本；aggregate层mean±std/paired delta/bootstrap；presentation层主表、补表、曲线、成功/失败图；每层有source_artifacts。
...
- 核心公式或定义参考：均值/样本标准差 `s=sqrt(Σ(x-mean)^2/(n-1))`；缺失不参与聚合且必须带missing_reason，禁止零填充。
- 代码落点：`scripts/summarize_stage.py::build_result_package()`；输入manifest列表，输出schema校验后的tables/figures manifest；当前不运行。
- 运行记录字段：见第7节。
- 验收方式：表格字段含dataset/split/method/identity/metric/direction/value_type/n/source；图含sample_id/selection_rule/panel/source hash。
...
- 接口：`scripts/summarize_stage.py::build_result_package()`；输入manifest列表，输出schema校验后的tables/figures manifest；当前不运行。
...
consumer_file: 10_结果汇总/01_结果包与统计协议.md
...
## 9. 验收、独立复核与handoff
- 验收：表格字段含dataset/split/method/identity/metric/direction/value_type/n/source；图含sample_id/selection_rule/panel/source hash。
...
## 10. 独立回退条件
raw不足退回07/08/09；aggregate失败不手工计算；缺失保留并降低claims。
