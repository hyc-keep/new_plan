# 07 raw、aggregate、paired delta与bootstrap schema
...
- 差异化角色依赖：同层00/01/03；下游10 raw/aggregate分层。
- 相关性说明：统计schema，区分训练重复与样本不确定性。
...
## 5. 核心规则卡片
- 当前结论：raw粒度=seed×split×sample×metric；aggregate=3 training seeds；paired delta同seed同sample；bootstrap单位=sample且固定声明不是训练重复。
...
- 核心公式或定义参考：`Δ_{s,i,m}=metric(N,s,i,m)-metric(B,s,i,m)`；`mean=Σ_s x_s/3`；样本bootstrap在样本索引有放回重采样，CI分位数预注册。
- 代码落点：`scripts/summarize_stage.py::aggregate_three_seeds()`、`bootstrap_paired_delta()`；输入raw CSV，输出planned aggregate schema。
- 运行记录字段：见第7节。
- 验收方式：TestA/B不合并；ObjHaus/HD95越低越好需方向归一字段；bootstrap不冒充显著性或增加n_seed。
...
- 接口：`scripts/summarize_stage.py::aggregate_three_seeds()`、`bootstrap_paired_delta()`；输入raw CSV，输出planned aggregate schema。
...
consumer_file: 07_正式GlaS/02_结果统计_schema.md
...
## 9. 验收、独立复核与handoff
- 验收：TestA/B不合并；ObjHaus/HD95越低越好需方向归一字段；bootstrap不冒充显著性或增加n_seed。
...
## 10. 独立回退条件
配对键不完整、seed数非3、split混合或CI单位错误时拒绝聚合，回退raw修复。
