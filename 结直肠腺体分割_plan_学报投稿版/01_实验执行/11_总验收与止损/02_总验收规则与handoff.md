...
- 差异化角色依赖：同层00/01；06-10全部manifest；投稿写作消费者。
- 相关性说明：最终schema和独立裁决文件，限定允许/禁止主张。

## 3. 路线依据与文献依据
- 路线依据：当前唯一编号链为 `05→06→07→08→09→10→11`；本文件位于 `11_总验收与止损`，不得跳阶段或把历史 archive 当 current Gate。
...
## 4. 本文件唯一变量与禁止重开项
- 唯一变量：只输出规划态Gate schema，当前decision=not_ready_not_run。
...
## 5. 核心规则卡片
- 当前结论：字段含decision、required/missing、stop_loss_action、claims_allowed/forbidden、limitations、B1 warning、blind audit、lineage和签署。
- 规则类型：`官方协议固定项 + 路线层已锁定 + 工程冻结规则`
- 适用阶段：`11_总验收与止损` 及其下游消费者。
- 直接依据：本文件第2-3节所列路线、文献和方法协议。
- 采用原因：保证单变量、盲态、公平、可追溯和有限预算。
- 不采用的相邻方案：测试集调参、运行中扩候选、混合协议并表、未注册后处理和无来源数字。
- 核心公式或定义参考：`decision∈{ready,not_ready,stop}`；当前无运行资产只能是`not_ready`，不得以文档齐全替代实验齐全。
- 代码落点：`src/registry/manifest_schema.py::validate_final_handoff()`；输入final gate dict，输出errors；写作只消费ready资产。
- 运行记录字段：见第7节。
- 验收方式：独立复核者逐项签署；missing_items为空且所有上游Gate真实通过才能ready；不承诺投稿录取。
...
- 接口：`src/registry/manifest_schema.py::validate_final_handoff()`；输入final gate dict，输出errors；写作只消费ready资产。
...
consumer_stage: 投稿写作或项目停止
consumer_file: 11_总验收与止损/02_总验收规则与handoff.md
...
## 9. 验收、独立复核与handoff
- 验收：独立复核者逐项签署；missing_items为空且所有上游Gate真实通过才能ready；不承诺投稿录取。
...
## 10. 独立回退条件
not_ready回缺失阶段；stop冻结路线并保存证据；任何新主张必须重开治理和预注册。
