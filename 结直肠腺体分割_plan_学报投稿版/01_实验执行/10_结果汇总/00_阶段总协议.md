...
- 差异化角色依赖：07/08/09 handoff；同层01/02；下游11。
- 相关性说明：汇总总览，严格区分raw、aggregate和展示资产。
...
## 5. 核心规则卡片
- 当前结论：raw不可改写；aggregate由脚本生成；display table/figure只引用aggregate或合规quoted；缺失值保持missing/not_run/blocked。
...
- 核心公式或定义参考：`display = f(raw, aggregation_spec, provenance)`；任何手工数字无source_artifact即拒绝。
- 代码落点：`scripts/summarize_stage.py`、`scripts/compare_runs.py`、`reports/tables/`、`reports/figures/` planned落点。
- 运行记录字段：见第7节。
- 验收方式：所有单元格可回链；TestA/B分表；CRAG、消融、成本、失败案例和外部身份完整。
...
- 接口：`scripts/summarize_stage.py`、`scripts/compare_runs.py`、`reports/tables/`、`reports/figures/` planned落点。
...
consumer_file: 10_结果汇总/00_阶段总协议.md
...
## 9. 验收、独立复核与handoff
- 验收：所有单元格可回链；TestA/B分表；CRAG、消融、成本、失败案例和外部身份完整。
