...
- 相关性说明：验收schema，保证来源真实性和可比性。

## 3. 路线依据与文献依据
- 路线依据：当前唯一编号链为 `05→06→07→08→09→10→11`；本文件位于 `09_外部对比`，不得跳阶段或把历史 archive 当 current Gate。
...
## 5. 核心规则卡片
- 当前结论：字段含method、identity_class、source/config、dataset/split/seed、metric identity、protocol difference、resource priority、path/citation、lineage。
- 规则类型：`官方协议固定项 + 路线层已锁定 + 工程冻结规则`
- 适用阶段：`09_外部对比` 及其下游消费者。
- 直接依据：本文件第2-3节所列路线、文献和方法协议。
- 采用原因：保证单变量、盲态、公平、可追溯和有限预算。
- 不采用的相邻方案：测试集调参、运行中扩候选、混合协议并表、未注册后处理和无来源数字。
- 核心公式或定义参考：`valid = reproduced⇒artifact_exists; quoted⇒citation_verified; reference_only⇒no_direct_rank_claim`。
- 代码落点：`src/registry/manifest_schema.py::validate_external_manifest()`；输入记录列表，输出分类错误和缺失项。
- 运行记录字段：见第7节。
- 验收方式：独立复核citation页码/表格或复现artifact；三类身份互斥；禁止空来源。
...
- 接口：`src/registry/manifest_schema.py::validate_external_manifest()`；输入记录列表，输出分类错误和缺失项。
...
consumer_stage: 09_外部对比
consumer_file: 09_外部对比/02_验收止损与manifest.md
...
## 9. 验收、独立复核与handoff
- 验收：独立复核citation页码/表格或复现artifact；三类身份互斥；禁止空来源。
...
## 10. 独立回退条件
复现失败可降级reference-only并记录原因；来源无法核实则删除该行，不猜数。
