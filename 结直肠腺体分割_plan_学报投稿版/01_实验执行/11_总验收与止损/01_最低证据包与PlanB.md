...
- 差异化角色依赖：路线05/06、05方法协议、06-10 handoff；同层00/02。
- 相关性说明：最低包和负向出口细则，统一Plan B名称与触发边界。
...
## 5. 核心规则卡片
- 当前结论：最低包含方法identity、N1 target合法性、N2去留、N3机制、GlaS三seed/TestA/B、CRAG、外部分栏、raw/统计、成本、成功失败案例、lineage。
...
- 核心公式或定义参考：Plan B统一为 `DCAN-inspired adaptation`，不是严格复现：仅N3 val机制失败且排除实现错误后一次，预算/候选预注册。
- 代码落点：`src/models/adaptations/dcan_inspired.py::build_dcan_inspired_adaptation(cfg)` 仅planned；输入区域/轮廓配置，输出联合推理概率/实例；不得声称存在。
- 运行记录字段：见第7节。
- 验收方式：若无正向结果：先删N2，允许N1+N3；N3失败才Plan B；仍失败收缩为N1辅助监督或停止方法主张。
...
- 接口：`src/models/adaptations/dcan_inspired.py::build_dcan_inspired_adaptation(cfg)` 仅planned；输入区域/轮廓配置，输出联合推理概率/实例；不得声称存在。
...
consumer_file: 11_总验收与止损/01_最低证据包与PlanB.md
...
## 9. 验收、独立复核与handoff
- 验收：若无正向结果：先删N2，允许N1+N3；N3失败才Plan B；仍失败收缩为N1辅助监督或停止方法主张。
...
## 10. 独立回退条件
禁止回旧LKMA/Distance、无限调参或看Test选回退；负结果保留为可审计证据。
