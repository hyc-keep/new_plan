...
- 差异化角色依赖：06 handoff；同层01-03；下游08、10、11。
- 相关性说明：正式三seed阶段总览，承接06唯一winner并向08和10交接。
...
## 5. 核心规则卡片
- 当前结论：seeds固定 `{3407,1234,2025}`；TestA60/TestB20物理分开；各seed checkpoint仅按val17 ObjDice最大选择。
...
- 核心公式或定义参考：`k*=argmax_k ObjDice_val17(k)`；三主指标 ObjF1/ObjDice/ObjHaus，四补充 Dice/IoU/HD95/BoundaryF1。
- 代码落点：`scripts/train.py`、`scripts/test.py`、`src/eval/run_eval.py`；三seed独立run bundle；TestA/B分别输出CSV。
- 运行记录字段：见第7节。
- 验收方式：三seed齐全、checkpoint规则一致、Test分表、raw可回链、独立复核通过，才handoff到08/10。
...
- 接口：`scripts/train.py`、`scripts/test.py`、`src/eval/run_eval.py`；三seed独立run bundle；TestA/B分别输出CSV。
...
consumer_file: 07_正式GlaS/00_阶段总协议.md
...
## 9. 验收、独立复核与handoff
- 验收：三seed齐全、checkpoint规则一致、Test分表、raw可回链、独立复核通过，才handoff到08/10。
...
## 10. 独立回退条件
任一seed协议漂移或Test反馈则该seed作废；不淘汰差seed，不重开06候选。
