...
- 差异化角色依赖：路线01/03/06；同层00/02；下游10表格。
- 相关性说明：资源与最低包细则，避免外部复现拖死主线。
...
## 5. 核心规则卡片
- 当前结论：P0=B1/UNet内部锚点；P1=UNet++、DeepLabV3+、Attention U-Net；P2=nnU-Net/Transformer或任务内增强。最低包=B1+至少两个公平复现经典方法，其余可quoted/reference-only。
...
- 核心公式或定义参考：`priority_score=representativeness/(implementation_cost+risk)` 仅作预注册排序，不根据结果改优先级。
- 代码落点：`external/*`适配层、`scripts/compare_runs.py`；记录implementation_source、license、adaptation、cost、result_tag。
- 运行记录字段：见第7节。
- 验收方式：最低包身份完整、至少两个reproduced且协议差异可解释；无法满足则如实标missing，不填数字。
...
- 接口：`external/*`适配层、`scripts/compare_runs.py`；记录implementation_source、license、adaptation、cost、result_tag。
...
consumer_file: 09_外部对比/01_优先级与最低包.md
...
## 9. 验收、独立复核与handoff
- 验收：最低包身份完整、至少两个reproduced且协议差异可解释；无法满足则如实标missing，不填数字。
...
## 10. 独立回退条件
资源不足按P2→P1尾部裁剪；不得删除B1身份或把quoted改成reproduced。
