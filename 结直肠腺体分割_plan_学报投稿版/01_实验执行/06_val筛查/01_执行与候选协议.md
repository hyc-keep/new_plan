...
- 差异化角色依赖：05的N1/N2/N3细则；同层00/02/03；下游07训练协议。
- 相关性说明：候选执行细则，突出注册表、预算计数和单变量顺序。
...
## 4. 本文件唯一变量与禁止重开项
- 唯一变量：只定义 candidate registry 和运行顺序。
...
## 5. 核心规则卡片
- 当前结论：N1 target宽度候选仅 `{1,2,3}`；N2仅 `concat+1x1/add/gate`；N3组合最多12；Plan B不占常规预算且仅机制门后一次。
...
- 核心公式或定义参考：预算 `B_total=Σ_c cost(c)`；每个候选 `changed_variable_count=1`；超预算自动拒绝而非追加。
- 代码落点：`src/registry/screening_registry.py::freeze_registry()` 与 `scripts/screen_candidates.py::run_candidate()`；输入 YAML，输出 candidate manifest、val raw和成本记录；当前均 planned。
- 运行记录字段：见第7节。
- 验收方式：注册时间早于首次运行；每个candidate_id唯一、parent_id存在、changed_variable单一、预算余额非负。
...
- 接口：`src/registry/screening_registry.py::freeze_registry()` 与 `scripts/screen_candidates.py::run_candidate()`；输入 YAML，输出 candidate manifest、val raw和成本记录；当前均 planned。
...
consumer_file: 06_val筛查/01_执行与候选协议.md
...
## 9. 验收、独立复核与handoff
- 验收：注册时间早于首次运行；每个candidate_id唯一、parent_id存在、changed_variable单一、预算余额非负。
...
## 10. 独立回退条件
注册冻结后发现缺项则整表作废并重新阶段锁定；N2无机制依据直接删除，不用test复活。
