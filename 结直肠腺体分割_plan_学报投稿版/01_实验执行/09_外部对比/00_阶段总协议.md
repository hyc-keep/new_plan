...
- 差异化角色依赖：07/08 handoff；同层01/02；下游10/11；外部方法文献提取稿。
- 相关性说明：外部比较总览，统一三种身份与公平边界。
...
## 4. 本文件唯一变量与禁止重开项
- 唯一变量：只登记 reproduced/quoted/reference-only，不制造等价性。
...
## 5. 核心规则卡片
- 当前结论：reproduced消费统一split/eval；quoted保留原论文协议；reference-only无可比数字或协议差异过大。
...
- 核心公式或定义参考：`fairness_grade = exact | adjusted_documented | non_comparable`；只有exact/合规adjusted可进入公平复现块。
- 代码落点：`scripts/compare_runs.py::classify_external_identity()`；输入run bundle或citation card，输出身份和protocol_difference。
- 运行记录字段：见第7节。
- 验收方式：每行来源、协议、指标、后处理、统计身份可回查；引用值绝不进入reproduced聚合。
...
- 接口：`scripts/compare_runs.py::classify_external_identity()`；输入run bundle或citation card，输出身份和protocol_difference。
...
consumer_file: 09_外部对比/00_阶段总协议.md
...
## 9. 验收、独立复核与handoff
- 验收：每行来源、协议、指标、后处理、统计身份可回查；引用值绝不进入reproduced聚合。
