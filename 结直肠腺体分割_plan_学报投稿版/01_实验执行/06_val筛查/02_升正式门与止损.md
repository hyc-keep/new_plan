...
- 差异化角色依赖：同层00/01/03；下游07总协议和11总Gate。
- 相关性说明：阶段验收文件，负责唯一选择、独立复核和06→07放行。
...
## 5. 核心规则卡片
- 当前结论：decision matrix必须包含 identity合法性、ObjF1/ObjDice/ObjHaus方向、Dice/IoU/HD95/BoundaryF1材料性退化、成本、失败模式、blind完整性。
...
- 核心公式或定义参考：`decision = promote_one | shrink | stop | trigger_plan_b_adaptation`；不设预承诺提升阈值，方向与机制综合裁决。
- 代码落点：`scripts/summarize_stage.py::build_screening_decision()`；输入registry、val raw、cost、blind audit，输出decision_matrix与handoff；由非运行者独立复核。
- 运行记录字段：见第7节。
- 验收方式：仅一个正式方法身份；复核者确认原始路径、选择规则和blind日志；否则Gate blocked。
...
- 接口：`scripts/summarize_stage.py::build_screening_decision()`；输入registry、val raw、cost、blind audit，输出decision_matrix与handoff；由非运行者独立复核。
...
consumer_file: 06_val筛查/02_升正式门与止损.md
...
## 9. 验收、独立复核与handoff
- 验收：仅一个正式方法身份；复核者确认原始路径、选择规则和blind日志；否则Gate blocked。
...
## 10. 独立回退条件
N1身份失败则停止；N2不足则删；N3机制失败才允许 `DCAN-inspired adaptation` 一次；仍失败收缩/停止。
