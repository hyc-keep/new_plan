# 10真实性验收、止损与table/figure manifest
...
- 差异化角色依赖：同层00/01；下游11总Gate。
- 相关性说明：汇总验收文件，逐单元格审计到原始资产。

## 3. 路线依据与文献依据
- 路线依据：当前唯一编号链为 `05→06→07→08→09→10→11`；本文件位于 `10_结果汇总`，不得跳阶段或把历史 archive 当 current Gate。
...
## 5. 核心规则卡片
- 当前结论：manifest包含table/figure id、source_artifacts、aggregation、seed_count、split、metric identity、identity class、generator、sha256、missing items。
- 规则类型：`官方协议固定项 + 路线层已锁定 + 工程冻结规则`
- 适用阶段：`10_结果汇总` 及其下游消费者。
- 直接依据：本文件第2-3节所列路线、文献和方法协议。
- 采用原因：保证单变量、盲态、公平、可追溯和有限预算。
- 不采用的相邻方案：测试集调参、运行中扩候选、混合协议并表、未注册后处理和无来源数字。
- 核心公式或定义参考：`traceable(cell)=source_path∧source_hash∧aggregation_spec`; 全表通过率必须100%，不是指标效果阈值。
- 代码落点：`scripts/summarize_stage.py::validate_result_manifest()`；输出错误清单与11 handoff。
- 运行记录字段：见第7节。
- 验收方式：quoted/reproduced分栏、缺失显式、图像选择规则预先定义、无手工数字。
...
- 接口：`scripts/summarize_stage.py::validate_result_manifest()`；输出错误清单与11 handoff。
...
consumer_stage: 10_结果汇总
consumer_file: 10_结果汇总/02_验收止损与manifest.md
...
## 9. 验收、独立复核与handoff
- 验收：quoted/reproduced分栏、缺失显式、图像选择规则预先定义、无手工数字。
...
## 10. 独立回退条件
任一不可回链单元格删除/标缺失并blocked；不因写稿需要伪造完整。
