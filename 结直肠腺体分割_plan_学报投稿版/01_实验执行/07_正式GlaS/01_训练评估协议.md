# 07三seed训练、checkpoint与Test blind协议
...
- 差异化角色依赖：06唯一handoff；同层00/02/03；下游08尺度协议。
- 相关性说明：训练评估细则，冻结selector、阈值和Test执行顺序。
...
## 5. 核心规则卡片
- 当前结论：best_selector=`val_objdice_max`；threshold_source=`val17`；TestA/B用同一冻结threshold/postprocess；评估前概率转float32。
...
- 核心公式或定义参考：`p=sigmoid(z_float32)`；`m=1[p≥t_val17]`；checkpoint为val ObjDice全epoch最大，平局取更早epoch。
- 代码落点：`scripts/train.py::main(cfg)`、`scripts/test.py::main(cfg)`；输入 `B×3×512×512 float32`，输出best.ckpt、val/TestA/TestB raw。
- 运行记录字段：见第7节。
- 验收方式：三个run的config digest除seed外一致；Test访问在checkpoint/threshold锁定后；独立复核hash。
...
- 接口：`scripts/train.py::main(cfg)`、`scripts/test.py::main(cfg)`；输入 `B×3×512×512 float32`，输出best.ckpt、val/TestA/TestB raw。
...
consumer_file: 07_正式GlaS/01_训练评估协议.md
...
## 9. 验收、独立复核与handoff
- 验收：三个run的config digest除seed外一致；Test访问在checkpoint/threshold锁定后；独立复核hash。
...
## 10. 独立回退条件
checkpoint缺失、selector不一致或Test调参则对应run无效并按原协议重跑，禁止用另seed替代。
