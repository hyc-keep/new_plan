...
- 差异化角色依赖：同层00/01；下游10/11。
- 相关性说明：验收schema，冻结test40盲态和下游证据身份。

## 3. 路线依据与文献依据
- 路线依据：当前唯一编号链为 `05→06→07→08→09→10→11`；本文件位于 `08_CRAG验证`，不得跳阶段或把历史 archive 当 current Gate。
...
## 5. 核心规则卡片
- 当前结论：manifest记录train/val/test、scale_source/candidates/selected、test_tuning_allowed=false、方法/config/seed/raw/失败案例。
- 规则类型：`官方协议固定项 + 路线层已锁定 + 工程冻结规则`
- 适用阶段：`08_CRAG验证` 及其下游消费者。
- 直接依据：本文件第2-3节所列路线、文献和方法协议。
- 采用原因：保证单变量、盲态、公平、可追溯和有限预算。
- 不采用的相邻方案：测试集调参、运行中扩候选、混合协议并表、未注册后处理和无来源数字。
- 核心公式或定义参考：`blind_ok = test_access_time > scale_lock_time ∧ test_tuning_allowed=false`。
- 代码落点：`src/registry/manifest_schema.py::validate_crag_manifest()`；输入manifest，输出errors和planned handoff。
- 运行记录字段：见第7节。
- 验收方式：train153/val20/test40计数、scale digest、test access log、raw路径、lineage均齐全。
...
- 接口：`src/registry/manifest_schema.py::validate_crag_manifest()`；输入manifest，输出errors和planned handoff。
...
consumer_stage: 08_CRAG验证
consumer_file: 08_CRAG验证/02_验收止损与manifest.md
...
## 9. 验收、独立复核与handoff
- 验收：train153/val20/test40计数、scale digest、test access log、raw路径、lineage均齐全。
...
## 10. 独立回退条件
缺尺度链或test泄漏则结果作废；域迁移不佳则stop并保留负向证据，不无限适配。
