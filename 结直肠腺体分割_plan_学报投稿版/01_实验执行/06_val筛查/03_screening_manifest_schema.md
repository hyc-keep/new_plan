# 06 screening manifest、blind与handoff schema
...
- 差异化角色依赖：同层00-02；下游07/10/11的manifest消费者。
- 相关性说明：schema文件，冻结机器可解析字段和工程阻断。

## 3. 路线依据与文献依据
- 路线依据：当前唯一编号链为 `05→06→07→08→09→10→11`；本文件位于 `06_val筛查`，不得跳阶段或把历史 archive 当 current Gate。
...
## 5. 核心规则卡片
- 当前结论：必填candidate、budget、selection、blind、lineage、artifact路径和decision字段；Test路径由访问控制拒绝。
- 规则类型：`官方协议固定项 + 路线层已锁定 + 工程冻结规则`
- 适用阶段：`06_val筛查` 及其下游消费者。
- 直接依据：本文件第2-3节所列路线、文献和方法协议。
- 采用原因：保证单变量、盲态、公平、可追溯和有限预算。
- 不采用的相邻方案：测试集调参、运行中扩候选、混合协议并表、未注册后处理和无来源数字。
- 核心公式或定义参考：`blind_ok = ¬testA_accessed ∧ ¬testB_accessed ∧ ¬crag_test_accessed`; 任一为false即作废。
- 代码落点：`src/registry/manifest_schema.py::validate_screening_manifest()`；输入dict，输出validation errors；无实验I/O。
- 运行记录字段：见第7节。
- 验收方式：schema可拒绝缺字段、重复winner、超预算、test访问、缺七字段lineage；06→07 handoff只能引用一个版本。
...
- 接口：`src/registry/manifest_schema.py::validate_screening_manifest()`；输入dict，输出validation errors；无实验I/O。
...
consumer_file: 06_val筛查/03_screening_manifest_schema.md
...
## 9. 验收、独立复核与handoff
- 验收：schema可拒绝缺字段、重复winner、超预算、test访问、缺七字段lineage；06→07 handoff只能引用一个版本。
