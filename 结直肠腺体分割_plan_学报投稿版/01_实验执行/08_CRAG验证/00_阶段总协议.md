...
- 差异化角色依赖：07方法/config handoff；同层01/02；下游10/11。
- 相关性说明：第二benchmark总协议，验证同任务迁移而非重开方法搜索。
...
## 4. 本文件唯一变量与禁止重开项
- 唯一变量：只在CRAG train153/val20适配训练与尺度，test40盲态一次评估。
...
## 5. 核心规则卡片
- 当前结论：数据固定 train153/val20/test40；GlaS参数不得默认照搬，结构身份不得因CRAG test改变。
...
- 核心公式或定义参考：`scale_ratio = statistic_CRAG_trainval/statistic_GlaS_trainval`，仅由train/val的像素间距或对象尺度统计得到。
- 代码落点：`scripts/train.py`、`scripts/test.py`、`src/data/scale_adapter.py` planned接口；输出CRAG run bundle与test_metrics.csv。
- 运行记录字段：见第7节。
- 验收方式：尺度来源可审计、test40未反馈、方法身份保持、失败案例齐全才handoff。
...
- 接口：`scripts/train.py`、`scripts/test.py`、`src/data/scale_adapter.py` planned接口；输出CRAG run bundle与test_metrics.csv。
...
consumer_file: 08_CRAG验证/00_阶段总协议.md
...
## 9. 验收、独立复核与handoff
- 验收：尺度来源可审计、test40未反馈、方法身份保持、失败案例齐全才handoff。
...
## 10. 独立回退条件
域迁移差可如实报告并停止；不依据test40扩候选或反向修改GlaS。
